  ...
  return (
    <>
      <div style={style} {...listeners} {...attributes} ref={setNodeRef}>
        <Card
          className={cn(
            'cursor-grab gap-4 p-3',
            isDragging && 'pointer-events-none cursor-grabbing opacity-30',
            className
          )}
        >
          {children ?? <p className="m-0 text-sm font-medium">{title}</p>}
        </Card>
      </div>
      {activeCardId === id && (
        <t.In>
...
export const KanbanCards = <T extends KanbanItemProps = KanbanItemProps>({
  ...
  return (
    <ScrollArea className="overflow-hidden py-2">
...
>({
  ...
  const announcements: Announcements = {
    ...
    onDragOver({ active, over }) {
      ...
      const newColumn = columns.find(column => column.id === over?.id)?.title
